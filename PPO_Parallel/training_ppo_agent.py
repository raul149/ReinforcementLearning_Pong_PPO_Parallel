...
def train(env_name, print_things=True, train_run_id=0, train_episodes=5000):
    ...
    for episode_number in range(train_episodes):
        ...
        identity = random.choice([0, 0, 0, 0, 0, 1, 1, 2, 3, 3])
        if identity==0:
            opponent = wimblepong.SimpleAi(env, opponent_id)
        if identity==1:
            opponent = Agent()
        ...
        if episode_number % 483 == 0 and episode_number != 0:
            torch.save(agent.policy.state_dict(), "PPOBASICv5%s_%d.mdl" % (env_name, episode_number))
