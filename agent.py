...
class Policy(torch.nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(4, 32, 8, stride=4),  # (number of layers, number of filters, kernel_size e.g. 8x8, stride)
            nn.ReLU(),
...
class Agent(object):
    ...
    def load_model(self):
        """ Load the model
        """
        weights = torch.load("PPOBASICv4WimblepongVisualSimpleAI-v0_9660.mdl", map_location='cpu')
